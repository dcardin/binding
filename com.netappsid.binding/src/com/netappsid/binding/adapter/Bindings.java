...
/**
 * Consists only of static methods that bind Swing components to ValueModels. This is one of two helper classes that assist you in establishing a binding: 1)
 * this Bindings class binds components that have been created before; it wraps ValueModels with the adapters from package
 * <code>com.jgoodies.binding.adapter</code>. 2) the BasicComponentFactory creates Swing components that are then bound using this Bindings class.
 * <p>
 * 
 * If you have an existing factory that vends Swing components, you can use this Bindings class to bind them to ValueModels. If you don't have such a factory,
 * you can use the BasicComponentFactory or a custom subclass to create and bind Swing components.
 * <p>
 * 
 * The binding process for JCheckBox, JRadioButton, and other AbstractButtons retains the former enablement state. Before the new (adapting) model is set, the
 * enablement is requested from the model, not the button. This enablement is set after the new model has been set.
 * <p>
 * 
 * TODO: Consider adding binding methods for JProgressBar, JSlider, JSpinner, and JTabbedPane.
 * <p>
 * 
 * TODO: Consider adding connection methods for pairs of bean properties. In addition to the PropertyConnector's {@code connect} method, this could add boolean
 * operators such as: not, and, or, nor.
 * 
...
public final class Bindings
{
	...
	/**
	 * A JComponent client property key used to store and retrieve an associated ComponentValueModel.
	 * 
	...
	/**
	 * A JComponent client property key used to store and retrieve an associated ComponentPropertyHandler.
	 * 
	 * @see #addComponentPropertyHandler(JComponent, ValueModel)
	 * @see #removeComponentPropertyHandler(JComponent)
	 */
	...
	/**
	 * Binds a JCheckBox to the given ValueModel and retains the enablement state. The bound check box is selected if and only if the model's value equals
	 ...
	 * 
	 * @param checkBox
	 *            the check box to be bound
	 * @param valueModel
	 *            the model that provides a Boolean value
	 * 
	 * @throws NullPointerException
	 *             if the checkBox or valueModel is {@code null}
	 */
	...
	/**
	 * Binds a JCheckBoxMenuItem to the given ValueModel and retains the enablement state. The bound menu item is selected if and only if the model's value
	 * equals <code>Boolean.TRUE</code>.
	 * <p>
	 * 
	 * <strong>Note:</strong> For users of the JGoodies UIF (user interface framework) the recommended way to create and bind check box menu items is the class
	 * <code>com.jgoodies.uif.ToggleAction</code>.
	 * <p>
	 * 
	 * The value model is converted to the required interface <code>ToggleButtonModel</code> using a <code>ToggleButtonAdapter</code>.
	 * 
	 * @param checkBoxMenuItem
	 *            the check box menu item to be bound
	 * @param valueModel
	 *            the model that provides a Boolean value
	 * 
	 * @throws NullPointerException
	 *             if the menu item or valueModel is {@code null}
	 */
	 ...
	 * <p>
	 * 
	 * Also, it is strongly recommended (though not required) that the underlying ValueModel provides only non-null values. This is so because the
	 * ColorSelectionModel behavior is undefined for {@code null} values and it may have unpredictable results. To avoid these problems, you may bind the
	 * ColorChooser with a default color using {@link #bind(JColorChooser, ValueModel, Color)}.
	 * <p>
	 * 
	 * Since the color chooser is considered a container, not a single component, it is not synchronized with the valueModel's {@link ComponentValueModel}
	 * properties - if any.
	 * <p>
	 * 
	 * <strong>Note:</strong> There's a bug in Java 1.4.2, Java 5 and Java 6 that affects this binding. The BasicColorChooserUI doesn't listen to changes in the
	 * selection model, and so the preview panel won't update if the selected color changes. As a workaround you can use
	 * {@link BasicComponentFactory#createColorChooser(ValueModel)}, or you could use a Look&amp;Feel that fixes the bug mentioned above.
	 * 
	 * @param colorChooser
	 *            the color chooser to be bound
	 * @param valueModel
	 *            the model that provides non-{@code null} Color values.
	 * 
	 * @throws NullPointerException
	 *             if the color chooser or value model is {@code null}
	 * 
	...
	/**
	 * Binds a JColorChooser to the given Color-typed ValueModel. The ValueModel must be of type Color and must allow read-access to its value. The default
	 ...
	 * 
	 * @throws NullPointerException
	 *             if the color chooser, value model, or default color is {@code null}
	 * 
	 * @since 1.1
	 */
	...
	/**
	 * Binds a non-editable JComboBox to the given SelectionInList using the SelectionInList's ListModel as list data provider and the SelectionInList's
	 ...
	 * <p>
	 * 
	 * Since version 2.0 the combo's <em>enabled</em> and <em>visible</em> state is synchronized with the selectionInList's selection holder, if it's a
	 * {@link ComponentValueModel}.
	 * 
	 * @param comboBox
	 *            the combo box to be bound
	 * @param selectionInList
	 *            provides the list and selection; if the selection holder is a ComponentValueModel, it is synchronized with the comboBox properties
	 *            <em>visible</em> and <em>enabled</em>
	 * @param <E>
	 *            the type of the combo box items
	 * 
	 * @throws NullPointerException
	 *             if the combo box or the selectionInList is {@code null}
	 * 
	 * @see ComboBoxAdapter
	 * 
	 * @since 1.0.1
	...
	/**
	 * Binds the given JFormattedTextField to the specified ValueModel. Synchronized the ValueModel's value with the text field's value by means of a
	 * PropertyConnector.
	 * 
	 * @param textField
	 ...
	 * 
	 * @throws NullPointerException
	 *             if the text field or valueModel is {@code null}
	 */
	 ...
	 * 
	 * @param label
	 *            a label that shall be bound to the given value model
	 * @param valueModel
	 *            the model that provides the value
	 * 
	 * @throws NullPointerException
	 *             if the label or valueModel is {@code null}
	 */
	...
	/**
	 * Binds a JList to the given SelectionInList using the SelectionInList's ListModel as list data provider and the SelectionInList's selection index holder
	 * for the selection model.
	 * <p>
	 * 
	 * Since version 2.0 the list's <em>enabled</em> and <em>visible</em> state is synchronized with the selectionInList's selection holder, if it's a
	 * {@link ComponentValueModel}.
	 * 
	 * @param list
	 *            the list to be bound
	 * @param selectionInList
	 *            provides the list and selection; if the selection holder is a ComponentValueModel, it is synchronized with the list properties
	 *            <em>visible</em> and <em>enabled</em>
	 * @param <E>
	 *            the type of the list items
	 * 
	 * @throws NullPointerException
	 *             if the list or the selectionInList is {@code null}
	 */
	...
	/**
	 * Binds a JRadioButton to the given ValueModel and retains the enablement state. The bound radio button is selected if and only if the model's value equals
	 * the specified choice value.
	 * <p>
	 * 
	 * The model is converted to the required interface ToggleButtonModel using a RadioButtonAdapter.
	 * 
	 * @param radioButton
	 *            the radio button to be bound to the given model
	 * @param model
	...
	/**
	 * Binds a JRadioButtonMenuItem to the given ValueModel and retains the enablement state. The bound menu item is selected if and only if the model's value
	 * equals the specified choice.
	 * <p>
	 * 
	 * <strong>Note:</strong> For users of the JGoodies UIF (user interface framework) the recommended way to create and bind radio button menu items is the
	 * class <code>com.jgoodies.uif.ToggleAction</code>.
	 * <p>
	 * 
	 * The model is converted to the required interface ToggleButtonModel using a RadioButtonAdapter.
	 * 
	 * @param radioButtonMenuItem
	 *            the radio item to be bound to the given model
	 * @param model
	 *            the model that provides the current choice
	 * @param choice
	 *            this button's value
	 ...
	 * <p>
	 * 
	 * TODO: Consider changing the semantics to commit on focus lost. This would be consistent with the text component vending factory methods in the
	 * BasicComponentFactory that have no boolean parameter.
	 * 
	 * @param textArea
	 *            the text area to be bound to the value model
	 * @param valueModel
	 *            the model that provides the text value
	 * 
	 * @throws NullPointerException
	 *             if the text component or valueModel is {@code null}
	 */
	...
	/**
	 * Binds a text area to the given ValueModel. The model can be updated on focus lost or on every character typed. The DocumentAdapter used in this binding
	 ...
	 * <p>
	 * 
	 * <strong>Security Note: </strong> If you use this method to bind a JPasswordField, the field's password will be requested as Strings from the field and
	 * will be held as String by the given ValueModel. These password String could potentially be observed in a security fraud. For stronger security it is
	 * recommended to request a character array from the JPasswordField and clear the array after use by setting each character to zero. Method
	 ...
	 * 
	 * @param textField
	 *            the text field to be bound to the value model
	 * @param valueModel
	 *            the model that provides the text value
	 * 
	 * @throws NullPointerException
	...
	/**
	 * Binds the specified property of the given JComponent to the specified ValueModel. Synchronizes the ValueModel's value with the component's property by
	 * means of a PropertyConnector.
	 * 
	 * @param component
	 *            the JComponent that is to be bound
	 * @param propertyName
	 ...
	 * 
	 * @throws NullPointerException
	 *             if the component or value model or property name is {@code null}
	 * 
	 * @since 1.2
	 */
	...
	/**
	 * If the given model is a ComponentValueModel, a component property handler is registered with this model. This handler updates the component state if the
	 * ComponentValueModel indicates a change in one of its properties, for example: <em>visible</em>, <em>enabled</em>, and <em>editable</em>.
	 * <p>
	 * 
	 * Also the ComponentValueModel and the component handler are stored as client properties with the component. This way they can be removed later using
	 * <code>#removeComponentPropertyHandler</code>.
	 * 
	 * @param component
	 *            the component where the handler is registered
	 * @param valueModel
	...
	/**
	 * If the given component holds a ComponentValueModel and a ComponentPropertyHandler in its client properties, the handler is removed as listener from the
	 * model, and the model and handler are removed from the client properties.
	 * 
	 * @param component
	 * 
	 * @see #addComponentPropertyHandler(JComponent, ValueModel)
	...
	/**
	 * Commits a pending edit - if any. Useful to ensure that edited values in bound text components that commit on focus-lost are committed before an operation
	...
	/**
	 * Flushes a pending edit in the focused text component - if any. Useful to revert edited values in bound text components that commit on focus-lost. This
	 * operation can be performed on an escape key event like the Cancel action in the JFormattedTextField.
	 * <p>
	 * 
	 * Returns whether an edit has been reset. Useful to decide whether a key event shall be consumed or not.
	 * 
	 * @return {@code true} if a pending edit has been reset, {@code false} if the focused component isn't buffering or doesn't buffer at all
	 * 
	...
	/**
	 * Checks and answers whether the focus owner is a component that buffers a pending edit. Useful to enable or disable a text component Action that resets
	 * the edited value.
	 * <p>
	 * 
	...
	/**
	 * Creates and returns a ValueModel that commits its value if the given component looses the focus permanently. It wraps the underlying ValueModel with a
	 * BufferedValueModel and delays the value commit until this class' shared FOCUS_LOST_TRIGGER commits. This happens, because this class' shared
	...
	/**
	 * Listens to property changes in a ComponentValueModel and updates the associated component state.
	 * 
	 * @see ComponentValueModel
	 */
	...
	private static final class WeakTrigger implements ValueModel
	{
		 ...
		 * <p>
		 * 
		 * In the rare case, where you want to notify a PropertyChangeListener even with PropertyChangeEvents that have no property name set, you can register
		 * the listener with #addPropertyChangeListener, not #addValueChangeListener.
		 * 
		 * @param listener
		 *            the listener to add
		 * 
		...
		/**
		 * Triggers a commit event in models that share this Trigger. Sets the value to {@code Boolean.TRUE} and ensures that listeners are notified about a
		 * value change to this new value. If necessary the value is temporarily set to {@code null}. This way it minimizes the number of PropertyChangeEvents
		...
		/**
		 * Triggers a flush event in models that share this Trigger. Sets the value to {@code Boolean.FALSE} and ensures that listeners are notified about a
		 * value change to this new value. If necessary the value is temporarily set to {@code null}. This way it minimizes the number of PropertyChangeEvents
		 * fired by this Trigger.
		 */
		void triggerFlush()
	...
	/**
	 * Differs from its superclass {@link PropertyChangeSupport} in that it uses WeakReferences for registering listeners. It wraps registered
	 * PropertyChangeListeners with instances of WeakPropertyChangeListener and cleans up a list of stale references when firing an event.
	 * <p>
	 * 
